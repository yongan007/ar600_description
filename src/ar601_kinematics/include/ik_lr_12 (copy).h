...
const double foot_z = 0.1055; //distance between floor and foot joints intersection
const double ankle_z = 0.1055; //distance between foot and ankle joints intersection 
const double leg_up = 0.28; //upper leg link length
...
void ik_left(double x, double y, double z, Ref<MatrixXd> R, double x_t, double y_t, double z_t, Ref<MatrixXd> Rt, Ref<VectorXd> angles)
{
    ...
    q6 = atan(yy / xx);
    q4 = -acos((pow(xx, 2) + pow(yy, 2) + pow(zz, 2)) / (2 * pow(0.28, 2)) - 1);
    q5 = -atan(sin(q4) / (1 + cos(q4))) + atan(zz / sqrt(pow(xx, 2) + pow(yy, 2)));
    Matrix4d T1;
    f_t1(q6, T1);
